                  ...
                  {subQuestion.question.map((option, optionIndex) =>
                    <div key={optionIndex} className='relative'>
                      <label htmlFor={`${subQuestion._id}${optionIndex + 1}`}
                        ...
                        ${(!disabled && data[`${question._id}_${subQuestion._id}`] == option) ? 'bg-yellow-500 transform scale-125 rounded-full' :
                            (!disabled && data[`${question._id}_${subQuestion._id}`] != option) ? 'bg-[#ef7931] rounded-lg hover:scale-105' :
